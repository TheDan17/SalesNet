...
public class DebugTaskController {
  @Autowired private final DebugTaskService debugTaskService;
  ...
  public ResponseEntity<Integer> createGetLogTask(
      @Valid @Min(1970) @RequestParam Short year,
    ...
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(debugTaskService.getLogNewTask(taskParams));
  }

  @Operation(summary = "Получение списка информации по всем существующим задачам")
  @ApiResponses({@ApiResponse(responseCode = "200", description = "Список задач получен")})
  ...
  public ResponseEntity<AsyncTaskInfo.Status> getStatus(
      @Valid @Min(1) @RequestParam Integer taskId) {
